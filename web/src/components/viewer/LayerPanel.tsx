...
/** Structural + Windows are one control: enabling it shows both. */
function StructuralRow() {
  const structural = useAppStore((s) => s.layers.structural);
  const setLayer = useAppStore((s) => s.setLayer);
  const toggle = (v: boolean) => { setLayer("structural", v); setLayer("fenestration", v); };
  return (
    <label className="flex cursor-pointer items-center gap-2 rounded-lg px-1.5 py-1 text-[13px] hover:bg-surface-muted">
      <input type="checkbox" checked={structural ?? false}
        onChange={(e) => toggle(e.target.checked)} className="h-3.5 w-3.5 accent-teal" />
      <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: LAYER_COLORS.structural }} />
      Structural + Windows
    </label>
  );
}
